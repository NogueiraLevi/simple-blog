...
def created_by(request, author_pk):
    user = User.objects.filter(pk=author_pk).firts()

    if user is None:
        raise Http404()

    posts = Post.objects.get_published()\
        .filter(created_by=author_pk)
...
class CreatedByListView(PostListView):
    ...
    def get_queryset(self) -> QuerySet[Any]:
        ...
        qs = qs.filter(created_by__pk=self.temp_context['user'].pk)
        return qs
...
def search(request):
    ...
    page_title = f'{search_value[:40]} - Search - '
